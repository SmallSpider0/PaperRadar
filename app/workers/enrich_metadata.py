...
def _venue_matches_name(venue: str, venue_code: str) -> bool:
    ...
    if venue_code == "ACM_CCS":
        return "computer and communications security" in venue or "sigsac conference on computer and communications security" in venue
    if venue_code == "NDSS":
        return "network and distributed system security symposium" in venue or venue == "ndss symposium"
    if venue_code == "USENIX_SECURITY":
        return "usenix security" in venue
    return False
...
def _fetch_openalex(title: str, venue_code: str, year: int) -> dict[str, Any] | None:
    resp = requests.get(
        OPENALEX_URL,
        params={"search": _fix_mojibake(title), "per-page": 10, "filter": f"publication_year:{year}"},
    ...
    best_score = 0.0
    for work in results:
        score = _title_similarity(title, work.get("title") or "")
        venue = ((work.get("primary_location") or {}).get("source") or {}).get("display_name") or ""
        if _venue_matches_name(venue, venue_code):
            score += 0.2
        if score > best_score:
            best = work
            best_score = score
...
def _fetch_crossref(title: str, venue_code: str, year: int) -> dict[str, Any] | None:
    resp = requests.get(
        CROSSREF_URL,
        params={"query.title": _fix_mojibake(title), "rows": 10, "filter": f"from-pub-date:{year}-01-01,until-pub-date:{year}-12-31"},
        headers={"User-Agent": USER_AGENT},
        timeout=30,
    )
    resp.raise_for_status()
    ...
    for item in items:
        ...
        if any(_venue_matches_name(name, venue_code) for name in (item.get("container-title") or [])):
            score += 0.2
        if score > best_score:
            best = item
            best_score = score
...
def _extract_ndss_page(record: dict[str, Any]) -> dict[str, Any]:
    html_text = fetch_html(record.get("paper_url", ""))
    ...
    if not record.get("authors_text"):
        ...
        if len(parts) == 2:
            record["authors_text"] = parts[0].strip() or record.get("authors_text")
            text = parts[1]
    if not record.get("abstract"):
        record["abstract"] = text[:5000].strip() or record.get("abstract")
    return record


def _extract_usenix_page(record: dict[str, Any]) -> dict[str, Any]:
    html_text = fetch_html(record.get("paper_url", ""))
    soup = BeautifulSoup(html_text, "lxml")

    if not record.get("authors_text"):
        citation_authors = [m.get("content", "").strip() for m in soup.select('meta[name="citation_author"]')]
        citation_authors = [name for name in citation_authors if name]
        if citation_authors:
            record["authors_text"] = ", ".join(citation_authors)

    if not record.get("abstract"):
        abstract_node = (
            soup.select_one("div.field-name-field-paper-description")
            or soup.select_one("div.field--name-field-paper-description")
            or soup.select_one("div.field-item.even")
        )
        if abstract_node:
            text = " ".join(abstract_node.get_text(" ", strip=True).split())
            if text:
...
def enrich_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if venue_code == "NDSS" and (not record.get("abstract") or not record.get("authors_text")):
        try:
            record = _extract_ndss_page(record)
        except Exception:
            pass

    if venue_code == "USENIX_SECURITY" and (not record.get("abstract") or not record.get("authors_text")):
        try:
            record = _extract_usenix_page(record)
        except Exception:
            pass

    if not record.get("abstract") or not record.get("authors_text"):
        try:
            work = _fetch_openalex(record.get("title", ""), venue_code, year)
        except Exception:
            work = None
        if work:
            if not record.get("abstract"):
    ...
    if not record.get("abstract") or not record.get("authors_text"):
        try:
            item = _fetch_crossref(record.get("title", ""), venue_code, year)
        except Exception:
            item = None
        if item:
            if not record.get("abstract"):
